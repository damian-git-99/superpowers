...
**In Pi:** Use `read` to load a skill's SKILL.md file. Skills are auto-discovered at startup — their names and descriptions appear in your system prompt. When a task matches a skill's description, load it with `read skills/<name>/SKILL.md` and follow its instructions.
...
Skills use Claude Code tool names. For Pi equivalents, see `references/pi-tools.md`.
...
digraph skill_flow {
    ...
    "Has checklist?" -> "Track progress" [label="yes"];
    "Has checklist?" -> "Follow skill exactly" [label="no"];
    "Track progress" -> "Follow skill exactly";
}
```
